...
public interface PaymentRepository extends JpaRepository<Payment, BigInteger> {
    ...
    List<Payment> findByMemberId(@Param("memberId") BigInteger memberId);
    
    /**
     * 콘서트별 결제 내역 조회
     */
    @Query("SELECT p FROM Payment p WHERE p.reservation.seat.concert.id = :concertId ORDER BY p.paidAt DESC")
    List<Payment> findByConcertId(@Param("concertId") BigInteger concertId);
     ...
     * 예매 ID로 결제 조회
     */
    Optional<Payment> findByReservationId(BigInteger reservationId);
    
    /**
     * 예매 ID로 결제 시도 횟수 조회
     */
    @Query("SELECT COUNT(p) FROM Payment p WHERE p.reservation.id = :reservationId")
    int countByReservationId(@Param("reservationId") BigInteger reservationId);
} 